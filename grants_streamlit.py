            ...
            if query not in displayed_parent_queries: # Prevent re-displaying the same parent query
                ...
                for j in range(i + 1, len(st.session_state.messages)):
                    if st.session_state.messages[j]["role"] == "assistant" and \
                       st.session_state.messages[j].get("parent_query") is None and \
                       st.session_state.messages[j]["content"] == st.session_state.messages[i+1]["content"]: # Crude way to link, ideally use a message ID or direct link
    ...
    if st.session_state.show_history:
        with st.sidebar:
            ...
            if not st.session_state.messages:
                ...
                # Group messages by original user query for history display
                # We need to filter out follow-up questions from the main history list
                # and only show the initial user query and its direct assistant response.
